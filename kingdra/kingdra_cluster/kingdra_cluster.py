...
class KingdraCluster(object):
    ...
    def fit(self, X_train, callback=None, cb2=None):
        ...
        graph_x     = [None for _ in range(n_en)]
            ...
            for en in range(n_en):
                ...
                if it == 0 or min_labels == 0:
                    ...
                    model.fit(x=[X_train, X_train_sup], y=Y_train_sup, batch_size=batch_size,
                              epochs=n_epochs_per_iter0, shuffle=False, verbose=1)  # ) callbacks=[lrate]  )
                else:
                    ...
                    model.fit(x=[X_train, X_train_sup], y=Y_train_sup, batch_size=batch_size,
                              epochs=n_epochs_per_iter, shuffle=True, verbose=1)  # ) callbacks=[lrate]  )
                graph_x[en] =model.z_model.predict_on_batch(x=X_train)
